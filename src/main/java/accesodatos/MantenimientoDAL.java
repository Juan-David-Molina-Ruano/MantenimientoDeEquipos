    ...
    public static int crear(Mantenimiento mantenimiento) {
        try (Connection conn = ComunDB.obtenerConexion()) {
            ...
            try (PreparedStatement statement = conn.prepareStatement(sql)) {
                String fecha = mantenimiento.getFecha(); // Suponiendo que fecha es una cadena en el formato "yyyy-MM-dd HH:mm:ss"
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"); // Define el formato de la fecha y hora
                LocalDateTime localDateTime = LocalDateTime.parse(fecha, formatter); // Convertir la cadena a LocalDateTime
                Timestamp fechstamp = Timestamp.valueOf(localDateTime); // Convertir LocalDateTime a java.sql.Timestamp
                statement.setTimestamp(4, fechstamp);
    ...
    public static int modificar(Mantenimiento mantenimiento) {
        try (Connection conn = ComunDB.obtenerConexion()) {
            ...
            try (PreparedStatement statement = conn.prepareStatement(sql)) {
                ...
                LocalDateTime localDateTime = LocalDateTime.parse(fecha, formatter); // Convertir la cadena a LocalDateTime
                Timestamp fechstamp = Timestamp.valueOf(localDateTime);
